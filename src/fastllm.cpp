...
// printf("repack %s (%s).\n", tensor->name.c_str(), ggml_type_name(tensor->type));
    ...
    //   1) 前 topk 次无条件入堆的 NaN(下标恰好是 0..topk-1)再也踢不出去;
    //   2) maxValue 变 NaN -> expf(x-NaN) 全 NaN -> psum/ps/curSum 全 NaN ->
    //      "curSum > rnd" 恒 false -> 一路落到 i==topk-1 返回低位下标;
    //   本模型 token 0 == '!', 1 == '"', 2 == '#', 所以表现为满屏感叹号。
    // 附带危害: 含 NaN 的比较器不满足严格弱序, std::sort / std::partial_sort
    //   在此属于未定义行为, libstdc++ 的 introsort 可能越界写。
    ...
    int SanitizeLogitsForSampling(float *base, int count, const char *where) {
        ...
        if (bad > 0) {
            long long n = ++g_nonFiniteLogitSteps;
            if (n <= 16 || (n % 100) == 0) {
                printf("[sampling] 非有限 logits: %s bad=%d/%d first=%d 累计=%lld%s\n",
                       where == nullptr ? "?" : where, bad, count, firstBad, (long long)n,
                       bad == count ? "  <<< 整层 logits 全废, 本步输出必然是垃圾" : "");
                fflush(stdout);
            }
        }
        return bad;
    ...
    int LLMSampling(Data &logits, int outerOffset,
        ...
        // expf(inf-inf) == NaN -> psum/ps/curSum 全 NaN -> "curSum > rnd"
        // 恒 false -> 落到兜底分支返回一个固定的低位 token, 输出退化成同一个
        // 字符的长串(实测 temperature=0 的请求返回 "44444444...")。
        // 正确语义: temperature <= 0 等价于贪心 argmax。
        // 写成 !(x > 0) 是为了同时挡住 NaN。
        const bool zeroTemperature = !(config.temperature > 0.0f);
        float invTemp = zeroTemperature ? 1.0f : 1.0f / config.temperature;
        ...
        topk = std::min(topk, (int)v.size());
        if (zeroTemperature) {
            // v 已按 betterThan 降序, v[0] 就是 argmax
    ...
    int LLMSamplingOnly(Data &logits, int outerOffset, const GenerationConfig &config) {
        ...
        // 同 LLMSampling: temperature<=0 走贪心, 不做除零。
        const bool zeroTemperature = !(config.temperature > 0.0f);
        float invTemp = zeroTemperature ? 1.0f : 1.0f / config.temperature;
        int topk = std::min(config.top_k, maxTopKSize);
        {
            // 这里是 (tokenId, logit) 交错对, 只净化 logit 分量;
            // tokenId 分量若非有限则整条候选不可用, 一并压掉。
            int bad = 0;
            for (int i = 0; i < maxTopKSize; i++) {
                if (!std::isfinite(base[i * 2 + 1]) || !std::isfinite(base[i * 2])) {
                    base[i * 2 + 1] = -1e30f;
                    bad++;
                }
            }
            if (bad > 0) {
                long long n = ++g_nonFiniteLogitSteps;
                if (n <= 16 || (n % 100) == 0) {
                    printf("[sampling] 非有限 logits: LLMSamplingOnly bad=%d/%d 累计=%lld%s\n",
                           bad, maxTopKSize, (long long)n,
                           bad == maxTopKSize ? "  <<< 整层候选全废" : "");
                    fflush(stdout);
                }
            }
        }
        ...
        if (zeroTemperature) {
            // 候选是 CUDA top-k 按分数降序给出的, 第一个合法候选即 argmax
            return base[candidateOffsets[0] * 2];
        }
    ...
    uint64_t GetPagedPoolCudaBytes() {
        long long v = g_pagedPoolCudaBytes.load(std::memory_order_relaxed);
        return v > 0 ? (uint64_t)v : 0;
    }

    bool GetVramBreakdown(VramBreakdown &out) {
#ifdef USE_CUDA
        // 用 fastllm 已有的封装, 避免在这里引 CUDA runtime 头
        const long long freeLL = FastllmCudaGetFreeSize();
        std::vector<long long> totals = FastllmCudaGetTotalSizes();
        if (freeLL <= 0 || totals.empty() || totals[0] <= 0) {
            return false;
        }
        const uint64_t freeB = (uint64_t)freeLL;
        const uint64_t totalB = (uint64_t)totals[0];
        uint64_t bigBusy = 0, bigFree = 0, smallBusy = 0, smallFree = 0,
                 pinned = 0;
        FastllmCudaMemoryBreakdown(&bigBusy, &bigFree, &smallBusy, &smallFree,
                                   &pinned);
        out.totalBytes = totalB;
        out.usedBytes = totalB > freeB ? totalB - freeB : 0;
        out.pagedPoolBytes = GetPagedPoolCudaBytes();
        out.allocBusyBytes = bigBusy + smallBusy;
        out.allocFreeBytes = bigFree + smallFree;
        out.graphPinnedBytes = pinned;
        return true;
#else
        (void)out;
        return false;
    ...
    void PagedCacheManager::Grow(int newMaxPages) {
        ...
        if (this->dataDevice == DataDevice::CUDA) {
            ...
            {
                // 需要的是"新池"这一块新增显存。旧池此刻**已经占着**显存,
                // 它不在 cudaMemGetInfo 的 free 里, 因此不该再被算进需求。
                // 原判据写成 newBytes + oldBytes, 把旧池重复计了一次, 在池子
                ...
                if (freeBytesLL > 0 && (uint64_t)freeBytesLL < needBytes) {
                    ...
                    // cudaMemGetInfo 看不见它们 —— 于是会出现"自己手里闲着几 GB,
                    // 却报显存不足"。先还回去再判。
                    // 此处刚做过 ForceDeviceSync(), 没有在途异步工作, 释放安全。
                    const long long beforeFree = freeBytesLL;
                    FastllmCudaClearBigBuffer();
                    freeBytesLL = FastllmCudaGetFreeSize();
                    printf("[PagedCache] Grow 显存不足, 回收缓存缓冲区: "
                           "free %lldMB -> %lldMB (需要 %lluMB)\n",
                           beforeFree / 1048576, freeBytesLL / 1048576,
                           (unsigned long long)(needBytes / 1048576));
                    fflush(stdout);
                }
                if (freeBytesLL > 0 && (uint64_t)freeBytesLL < needBytes) {
                    ErrorInFastLLM(
