...
// printf("repack %s (%s).\n", tensor->name.c_str(), ggml_type_name(tensor->type));
    ...
    void PrefixCacheStatsObserveRequest(
        ...
        if (reqSeq % 8 == 0) {
            ...
            printf("[PrefixCache] periodic: reqs=%llu hitReqs=%llu "
                   ...
                   "gen=%llu restore-fail=%llu extra-missing=%llu "
                   "other=%llu} "
    ...
    int PagedCacheManager::GetUnusedPageIndex(bool pick) {
        ...
        if (pageIndex < 0) {
            ...
            // 无需各自处理页不足。
            // TryGrow 不抛错：扩容失败时先把最冷的前缀页下沉到 L2/L3
    ...
    bool PagedCacheManager::PageOutTrieNode(
        ...
        // 4096 tok = 32 页。原默认 65536 意味着"只有 64K 以上的前缀才配
        // 下沉"，而 agent 每轮 20~130K 的前缀链上绝大多数节点
        // maxPrefixDepthPages*pageLen 达不到 64K -> 一律被拒 -> 页被直接
        // 删掉而不是下沉，L2/L3 永远是空的。
        ...
        // ---- L2: RAM 层优先 ----
        // 正确的降级链是 VRAM -> RAM -> disk。原实现在 DISK_DIR 有配置时
        // 先写盘,RAM 层永远拿不到数据(resident{cpu=0MB}),而且冷页要回捞
        // 时必须走一次磁盘读。这里改成: 先尝试 RAM,RAM 满/未启用才落盘。
        bool cpuTierFull = false;
        if (PagedPrefixCacheCpuTierEnabled()) {
            ...
            if (reservationOk && maxCpuBytes != 0) {
                // RAM 满：把最冷的 RAM payload 先沉到 disk 腾位置,
                // 这才是"逐级往下挪"。原来只会直接改写盘, RAM 层一旦
                // 撞到 CPU_MAX_BYTES 就永久卡死在满的状态。
                const uint64_t current =
                    pagedPrefixCacheCpuTierBytes.load();
                if (current > maxCpuBytes ||
                    payloadBytes > maxCpuBytes - current) {
                    this->DemoteCpuTierPayloadsToDisk(payloadBytes);
                }
            }
            if (reservationOk) {
                uint64_t current =
        ...
        // ---- L3: disk 层 ----
        // RAM 满(或未启用)时继续往下沉,而不是把这一页直接丢掉。
        // 默认门槛与 RAM 层对齐: 原 65536 让磁盘层也永远收不到数据。
